...
public class StupidPlayer implements Player
{
	...
	public int lookAndMove( int[][] aintViewL, int[][] aintViewR )
	{
		...
		if( round ==1) {
			...
			state = new short[maxVirtualMapsize][maxVirtualMapsize][maxVirtualMapsize][maxVirtualMapsize];
		}

		updateLeftRightMap(aintViewL,aintViewR);

		if (movesList.isEmpty()) {
			//System.out.println("movelist empty");
			// do we have to do above for all rounds 
			// initialize state[][][][] instead of creating a new one
			for(int i=0;i<maxVirtualMapsize;i++){
				for(int j=0;j<maxVirtualMapsize;j++){
					for(int k=0;k<maxVirtualMapsize;k++){
						for(int m=0;m<maxVirtualMapsize;m++){
							state[i][j][k][m]=0;
						}
					}
				}
		...
		while(!queueElements.isEmpty()){
			...
			for(int moves=1;moves<=8;moves++) {
				...
				if (state[xm1][ym1][xm2][ym2]==0 ) { 
					...
					state[xm1][ym1][xm2][ym2]=(short) (moves*100+leftSame*10+rightSame);
